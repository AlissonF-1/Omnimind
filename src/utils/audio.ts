export function playTrophySound() {
  ...
  try {
    ...
    // Arpejo de C Major (C5 -> E5 -> G5 -> C6) sustenido
    const notes = [523.25, 659.25, 783.99, 1046.50]
    notes.forEach((freq, idx) => {
      playNote(freq, idx * 0.08, 0.8)
    })
...
export function playButtonClick(rating: number) {
  if (typeof window === 'undefined') return
  try {
    const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext
    if (!AudioContextClass) return
    ...
    let freq = 500
    if (rating === 1) freq = 300 // Again
    else if (rating === 2) freq = 420 // Hard
    else if (rating === 3) freq = 550 // Good
    else if (rating === 4) freq = 800 // Easy
    ...
    console.warn('Button click sound error', e)
  }
}

export function playEvaluationSound(correct: boolean) {
  if (typeof window === 'undefined') return
  try {
    const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext
    if (!AudioContextClass) return
    ...
    if (correct) {
      // Arpejo ascendente de 3 notas de acerto (C5 -> E5 -> G5)
      ...
      // Baixo sutil descendente de erro (sofisticado e discreto, nada irritante)
      const osc = ctx.createOscillator()
      const gain = ctx.createGain()
      osc.connect(gain)
      gain.connect(ctx.destination)
    ...
    console.warn('Evaluation sound error', e)
  }
}
